...
abstract class BaseEnum
{
    ...
    public static function createByName($name)
    {
        ...
        if (!array_key_exists($name, $constants)) {
            throw new UnexpectedValueException(
                "Name '{$name}' is not exists in the enum constants list " . get_called_class()
            );
        }
    ...
    public static function createByValue($value)
    {
        if (!array_key_exists($value, static::getConstantsByValue())) {
            throw new UnexpectedValueException(
                "Value '{$value}' is not exists in the enum constants list " . get_called_class()
            );
        }
    ...
    {
        return array_map(fn($value) => __($value), static::$list);
    }
